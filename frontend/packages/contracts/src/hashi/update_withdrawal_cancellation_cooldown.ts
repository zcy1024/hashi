...
const $moduleName = "@local-pkg/hashi::update_withdrawal_cancellation_cooldown";
export const UpdateWithdrawalCancellationCooldown = new MoveStruct({
  name: `${$moduleName}::UpdateWithdrawalCancellationCooldown`,
  fields: {
    cooldown_ms: bcs.u64(),
  },
});
...
export function propose(options: ProposeOptions) {
  ...
  const parameterNames = ["hashi", "cooldownMs", "metadata"];
  return (tx: Transaction) =>
    tx.moveCall({
      package: packageAddress,
      module: "update_withdrawal_cancellation_cooldown",
      function: "propose",
